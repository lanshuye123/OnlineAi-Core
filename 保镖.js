...
Core.AddListener((connect, info) => {
    if (info.message == "聘请保镖") {
        if (Money.add.GetUserMoney(Core.GetUser(info.user_id)) < 保镖配置.聘请定价) {
        ...
        Money.add.CostUserMoney(Core.GetUser(info.user_id), 保镖配置.聘请定价);
        User保镖 = User保镖 + 保镖配置.使用次数;
        Data[Core.GetUser(info.user_id)] = User保镖;
        Core.frame.WriteSystemConfig("保镖", Data);
        Core.frame.SendMsg(connect, info, `${Core.frame.At(Core.GetUser(info.user_id))},聘请保镖成功,你剩下${User保镖}位保镖,还剩${Money.add.GetUserMoney(Core.GetUser(info.user_id))}RMB.`);
    }
});
